...
def LoadFaceImages(pathname, subject_name, num_images):
    ...
    if(subset == True):
        ...
        for i in im_list:
            shadow = load_image(i)
            num_shadow = len(np.where(shadow < 5)[0])  #numbers where shadow intensity < 10
            if(num_shadow / shadow.size < 0.1):
                im_sub_list.append(i)
    else:
...
def preprocess(ambimage, imarray):
    ...
    """
    sub = imarray - ambimage[:,:,np.newaxis]
    sub[sub < 0] = 0
    processed_imarray = sub/255
...
def get_surface(surface_normals, integration_method):
    ...
    Outputs:
        height_map: h x w
    """
    derx = surface_normals[:,:,0]/surface_normals[:,:,2]
    dery = surface_normals[:,:,1]/surface_normals[:,:,2]
    intx = np.cumsum(derx, axis=1)
    inty = np.cumsum(dery, axis=0)
    H, W, _ = surface_normals.shape
    if(integration_method == 'row'):
        return intx[0,:]+inty
    elif(integration_method == 'column'):
        return intx+inty[:,0][:,np.newaxis]
    elif(integration_method == 'average'):
        return (intx[0,:]+inty+intx+inty[:,0][:,np.newaxis])/2
